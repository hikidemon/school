    ...
    <label class="switch">
      ...
      <span class="slider">
        <transition name="fade-slide" mode="out-in">
          <component :is="isDarkTheme ? moon : emerald" class="theme-icon" :class="{ 'dark-icon': isDarkTheme }"
            key="theme-icon" />
        </transition>
      </span>
    </label>
    <div class="profile" @mouseenter="expandAvatar" @mouseleave="shrinkAvatar">
      <a-image :src="user.avatarUrl || Avatar" alt="Профиль"
        :class="['avatar', { 'avatar-expanded': isAvatarExpanded }]" @click="toggleDropdown" />
